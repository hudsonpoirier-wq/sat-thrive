...
export default function Admin() {
  ...
  useEffect(() => {
    ...
    Promise.all([
      ...
      supabase.from('test_attempts').select('*').eq('completed_at', null).not('completed_at', 'is', null).order('started_at', { ascending: false }),
      supabase.from('post_scores').select('*')
    ]).then(([p, a, ps]) => {
      // Get all completed attempts
      supabase.from('test_attempts').select('*').not('completed_at', 'is', null).order('started_at', { ascending: false })
        .then(({ data: completedAttempts }) => {
          setStudents(p.data || [])
  ...
  // Build paired data
  const pairs = attempts.map(a => {
    const post = postScores.find(p => p.attempt_id === a.id)
    if (!post) return null
    return { pre: a.scores?.total, post: post.post_score, name: students.find(s => s.id === a.user_id)?.full_name || 'Unknown' }
  }).filter(Boolean)
            ...
            <table style={{ width: '100%', borderCollapse: 'collapse' }}>
              ...
              <tbody>
                {students.map(s => {
                  ...
                  const best = userAttempts.length ? Math.max(...userAttempts.map(a => a.scores?.total || 0)) : null
                  return (
