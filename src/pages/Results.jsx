...
export default function Results() {
  ...
  async function generatePlan() {
    ...
    const topicsText = weakTopics.slice(0, 10).map(t =>
      `- ${t.name} (Playbook Ch ${t.ch}, p.${t.page}): ${t.count} question(s) missed`
    ).join('\n')
    ...
    try {
      const r = await fetch('https://api.anthropic.com/v1/messages', {
        ...
        body: JSON.stringify({
          ...
          messages: [{
            role: 'user',
            content: `You are an elite SAT tutor for SAT Thrive, a nonprofit helping underserved students reach 1550+.

Student Pre-Test Score: ${scores.total} (R&W: ${scores.rw}, Math: ${scores.math})

Weakest areas identified from wrong answers:
${topicsText || 'No specific weaknesses logged'}

Generate a focused 8-week custom study plan using ONLY the SAT Thrive Playbook. Reference exact chapter numbers and page numbers.

FORMAT:

▶ SCORE ASSESSMENT (2 sentences — where they are and what's realistic in 8 weeks)

🔴 TOP 3 PRIORITY FIXES
1. [Topic] — Ch X, Playbook p.X — [why it matters most]
2. [Topic] — Ch X, Playbook p.X — [why]
3. [Topic] — Ch X, Playbook p.X — [why]

📅 8-WEEK ROADMAP
Weeks 1–2: [Specific chapters + pages to study, 1 timed drill]
Weeks 3–4: [Specific chapters + pages, 1 timed drill]
Weeks 5–6: [Specific chapters + pages, 1 timed drill]
Weeks 7–8: [Full review + test simulation]

📌 TUTOR NOTES
• [Error pattern 1 — actionable instruction]
• [Error pattern 2 — actionable instruction]
• [Error pattern 3 — what to watch]

Be specific, direct, and data-driven. Reference Playbook pages every time.`
          }]
        })
      })
      const d = await r.json()
      const txt = d.content?.find(b => b.type === 'text')?.text || 'Error generating plan.'
      setPlan(txt)
      await supabase.from('test_attempts').update({ study_plan: txt }).eq('id', attemptId)
      setPlanSaved(true)
    } catch (e) {
      setPlan('Connection error — check your API key in .env')
    }
